...
CMD:poster ( playerid, params [] ) {
    ...
    if ( sscanf ( params, "iiis[24]", skin, price, jailtime, name )) {
        ...
        return SendClientMessage(playerid, -1, "/poster [skin_id] [ödül_miktarý] [hapis_süresi] [isim]") ;
    }

    if ( strlen ( name ) > MAX_PLAYER_NAME) {

        return  SendClientMessage(playerid, -1, "Ýsim 24 karakterden uzun olamaz!") ;
    }

    if ( ! IsValidSkin ( skin ) ) {

        return  SendClientMessage(playerid, -1, "Geçerli bir skin seçmelisin!") ;
    }

    if ( Character [ playerid ] [ character_handmoney ] < price ) {

        return SendServerMessage ( playerid, "Bu ödülü koymak için yeterli paran yok.", MSG_TYPE_ERROR ) ;
    }
...
CMD:removeposter ( playerid, params [] ) {
    ...
    if ( sscanf ( params, "D(-1)", posterid ) ) {
        ...
        return SendServerMessage ( playerid, "/removeposter [opsiyonel: poster_id]", MSG_TYPE_ERROR ) ;
    }
    ...
    else return SendServerMessage(playerid, "Bir arananlar ilanýnýn yakýnýnda deðilsin.", MSG_TYPE_ERROR ) ;
...
CMD:takebounty ( playerid, params [] ) {
    ...
    if ( sscanf ( params, "D(-1)", poster ) ) {
        ...
        return SendServerMessage ( playerid, "/takebounty [opsiyonel: poster_id]", MSG_TYPE_ERROR ) ;
    }

    if ( Character [ playerid ] [ character_bounty_id ] != -1 ) {

        return SendServerMessage ( playerid, "Zaten bir ödül avý görevi kabul etmiþsin!", MSG_TYPE_ERROR ) ;
    }
    ...
    if ( IsPlayerNearWantedPoster ( playerid ) != -1 ) {
        ...
        SendServerMessage ( playerid, sprintf("%s kiþisinin ödül avýný kabul ettin.", WantedPoster [ poster ] [ poster_name ] ), MSG_TYPE_INFO ) ;
    }
...
CMD:abandonbounty ( playerid, params [] ) {
    ...
    if ( ! Character [ playerid ] [ character_bounty_id ] ) {
        ...
        return SendServerMessage ( playerid, "Kabul ettiðin bir ödül avý yok!", MSG_TYPE_ERROR ) ;
    }
    ...
    SendServerMessage ( playerid, "Ödül avý görevini býraktýn.", MSG_TYPE_INFO ) ;
...
CMD:claimbounty ( playerid, params [] ) {
    ...
    if ( player_BountyKill [ playerid ] == -1 ) { 
        ...
        if ( bountyid == INVALID_PLAYER_ID ) {
            ...
            return SendServerMessage ( playerid, "Ödül avýndaki kiþi þu an çevrimdýþý veya bir senkronizasyon hatasý var.", MSG_TYPE_ERROR ) ;
        }
        ...
        if ( yds > 5.0 ) {
            ...
            return SendServerMessage ( playerid, "Ödül avýndaki kiþiye yakýn deðilsin.", MSG_TYPE_ERROR ) ;
        }

        SendServerMessage ( bountyid, "Ödül avý nedeniyle hapse atýldýn.", MSG_TYPE_INFO ) ;
        ...
        SendServerMessage ( playerid, sprintf("Ödül avý için $%s ödeme aldýn.", IntegerWithDelimiter ( WantedPoster [ posterid ] [ poster_price ] ) ), MSG_TYPE_INFO ) ;
        ...
        if ( CarryingBountyCorpse [ playerid ] == false ) {
            ...
            return SendServerMessage ( playerid, "Cesedi taþýyor olmalýsýn.", MSG_TYPE_ERROR ) ;
        }
...
CMD:pickbounty ( playerid, params [] ) {
    ...
    if ( bountyid == -1 ) {
        ...
        return SendServerMessage ( playerid, "Ödül avýndaki kiþiyi öldürmedin.", MSG_TYPE_ERROR ) ;
    }
    ...
    if ( IsValidBounty ( playerid, Corpse [ bountyid ] [ corpse_pid ] ) ) {
        ...
        SendServerMessage ( playerid, sprintf("%s kiþisinin cesedini yerden aldýn.", ReturnUserName ( Corpse [ bountyid ] [ corpse_pid ], false ) ), MSG_TYPE_INFO ) ;
    }

    else return SendServerMessage ( playerid, "Doðru cesedin yakýnýnda deðilsin!", MSG_TYPE_ERROR ) ;
...
public OnPlayerKeyStateChange ( playerid, KEY: newkeys, KEY: oldkeys ) {
    ...
    if ( newkeys & KEY_WALK && IsPlayerNearWantedPoster ( playerid ) != -1 ) {
        ...
        if ( IsPlayerViewingInventory [ playerid ] ) {
            ...
            return SendServerMessage ( playerid, "Envanterin açýkken arananlar ilanýna bakamazsýn!", MSG_TYPE_ERROR ) ;
        }
