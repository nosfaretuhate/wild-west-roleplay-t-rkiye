...
CMD:td(playerid, params [] ) {
    ...
    if ( DoesPlayerHaveItemByExtraParam ( playerid, RADIO ) == -1 ) {
        ...
        return SendServerMessage ( playerid, "Bu komutu kullanmak için bir telsiz cihazýna ihtiyacýn var.", MSG_TYPE_INFO ) ;
    }

    if ( sscanf ( params, "s[144]", text ) ) {

        return SendServerMessage(playerid, "/td (/r) [mesaj]", MSG_TYPE_ERROR ) ;
    }

    if ( strlen ( text ) > 144 ) {

        return SendServerMessage(playerid, "Mesajýn 144 karakterden uzun olamaz.", MSG_TYPE_ERROR ) ;
    }
...
SendTransmittorMessage ( playerid, text [] ) {
    ...
    if ( yds == - 1.0 ) {
        ...
        return SendClientMessage(playerid, -1, "Gönderdiðin mesaj cýzýrtý ile karþýlýk buldu. Görünüþe göre bir iletim bölgesinde deðilsin." ) ;
    }

    SendSplitMessage(playerid, 0xCCAB7CFF, sprintf("** {96713E}[Telsiz]{CCAB7C} %s: %s **", ReturnUserName ( playerid, false ), text ) );
        ...
        if ( IsLawEnforcementPosse ( Character [ i ] [ character_posse ] ) ) {
            ...
            if ( DoesPlayerHaveItemByExtraParam ( i, RADIO ) ) {
                ...
                if ( dist != -1.0 ) {
                    ...
                    SendSplitMessage(i, 0xCCAB7CFF, sprintf("** {96713E}[Telsiz]{CCAB7C} %s: %s **", ReturnUserName ( playerid, false ), rippled_text ) );
                }
                else continue;
            }
            else continue;
