...
CMD:showinjuries ( playerid, params [] ) {
    ...
    if ( sscanf ( params, "k<u>", target ) ) {
        ...
        return SendServerMessage ( playerid, "/showinjuries [oyuncuid]", MSG_TYPE_ERROR ) ;
    }

    if ( ! IsPlayerConnected( target ) ) {

        return SendServerMessage ( playerid, "Oyuncu baðlý deðil! Tekrar dene.", MSG_TYPE_ERROR ) ;
    }

    new string [ 512 ], count ;

    strcat(string, "Vücut Bölgesi\tYara Tipi\tMiktar\n" ) ;
    ...
    if ( count == 0 ) {
        ...
        return SendServerMessage ( playerid, sprintf("%s (%d) adlý oyuncuda görünür bir yara yok.", ReturnUserName ( target, false ), target ), MSG_TYPE_ERROR ) ;
        
    }

    ShowPlayerDialog(playerid, 546, DIALOG_STYLE_TABLIST_HEADERS, sprintf("%s (%d) - Yaralar\n", ReturnUserName ( target, false ), target ), string, "Kapat", "" ) ;
...
GetWoundType ( weaponid ) {
    ...
    switch ( weaponid ) {
        ...
        case 1 .. 3, 5 .. 7, 10 .. 15: woundtype = "Künt Travma" ;
        case 4, 8 .. 9: woundtype = "Kesici Alet" ;
        case 22 .. 34: woundtype = "Ateþli Silah" ;
        case 18, 35 .. 37, 16, 39 .. 40: woundtype = "Yanýk" ;
        default: woundtype = "Bilinmiyor";
    }
...
GetBodyPartName ( bodypart ) {
    ...
    switch ( bodypart ) {
        case BODY_PART_GROIN:       bodypartname = "Kasýk" ;
        case BODY_PART_TORSO:       bodypartname = "Gövde" ;
        case BODY_PART_LEFT_ARM:    bodypartname = "Sol Kol" ;
        case BODY_PART_RIGHT_ARM:   bodypartname = "Sað Kol" ;
        case BODY_PART_LEFT_LEG:    bodypartname = "Sol Bacak" ;
        case BODY_PART_RIGHT_LEG:   bodypartname = "Sað Bacak" ;
        case BODY_PART_HEAD:        bodypartname = "Baþ" ;
    }
