...
stock Poker_ReturnPlayerTable ( playerid ) {
...
stock Poker_ReturnPlayerHandle(playerid) {

	return Poker_ReturnPlayerTable(playerid);
}
	...
	inline Load_PokerTables() {
		...
		if(rows) {
			...
			for(new i=0; i<rows; i++) {
				...
				new buy_in,small_blind,Float:x,Float:y,Float:z,seats,vworld,interior;

				cache_get_value_name_int(i,"poker_table_buy_in",buy_in);
				cache_get_value_name_int(i,"poker_table_small_blind",small_blind);
				cache_get_value_name_float(i,"poker_table_x_pos",x);
				cache_get_value_name_float(i,"poker_table_y_pos",y);
				cache_get_value_name_float(i,"poker_table_z_pos",z);
				cache_get_value_name_int(i,"poker_table_seats",seats);
				cache_get_value_name_int(i,"poker_table_vw",vworld);
				cache_get_value_name_int(i,"poker_table_interior",interior);

				CreatePokerTable(buy_in,small_blind,x,y,z,seats,vworld,interior);
			}

			printf("[POKER] %d poker tables loaded.",rows);
		}
	}
	MySQL_TQueryInline(mysql,using inline Load_PokerTables,"SELECT * FROM poker");
	return true;
}

stock CreatePokerTable(buy_in, small_blind, Float: X, Float: Y, Float: Z, seats, vworld, interior, db = 0)
{
...
stock KickPlayerFromTable(playerid)
{
	...
	GiveCharacterMoney(playerid, PlayerPokerData[playerid][E_PLAYER_TOTAL_CHIPS]);
	...
	Safe_SetPlayerPos(playerid, GetPVarFloat(playerid, "t_temp_posX"), GetPVarFloat(playerid, "t_temp_posY"), GetPVarFloat(playerid, "t_temp_posZ"));
...
stock Player_CreateTextdraws(playerid)
{
	...
	for(new i = 0; i < TableData[handle][E_TABLE_TOTAL_SEATS]; i++)
	{
		...
		PlayerTextDrawFont(playerid, PlayerPokerData[playerid][E_PLAYER_CARDS_TXT_1][i], TEXT_DRAW_FONT_SPRITE_DRAW);
		...
		PlayerTextDrawFont(playerid, PlayerPokerData[playerid][E_PLAYER_CARDS_TXT_2][i], TEXT_DRAW_FONT_SPRITE_DRAW);
	}
	//Community cards
	for(new i = 0; i < 5; i++)
	{
		...
		PlayerTextDrawFont(playerid, PlayerPokerData[playerid][E_PLAYER_COMMUNITY_CARDS_TXT][i], TEXT_DRAW_FONT_SPRITE_DRAW);
	}
	...
	PlayerTextDrawAlignment(playerid, PlayerPokerData[playerid][E_PLAYER_CHOICES_TXT][CALL], TEXT_DRAW_ALIGN_CENTRE);
	...
	PlayerTextDrawUseBox(playerid, PlayerPokerData[playerid][E_PLAYER_CHOICES_TXT][CALL], true);
	...
	PlayerTextDrawAlignment(playerid, PlayerPokerData[playerid][E_PLAYER_CHOICES_TXT][RAISE], TEXT_DRAW_ALIGN_CENTRE);
	...
	PlayerTextDrawUseBox(playerid, PlayerPokerData[playerid][E_PLAYER_CHOICES_TXT][RAISE], true);
	...
	PlayerTextDrawSetSelectable(playerid, PlayerPokerData[playerid][E_PLAYER_CHOICES_TXT][RAISE], true);
	...
	PlayerTextDrawAlignment(playerid, PlayerPokerData[playerid][E_PLAYER_CHOICES_TXT][FOLD], TEXT_DRAW_ALIGN_CENTRE);
	...
	PlayerTextDrawUseBox(playerid, PlayerPokerData[playerid][E_PLAYER_CHOICES_TXT][FOLD], true);
	...
	PlayerTextDrawSetSelectable(playerid, PlayerPokerData[playerid][E_PLAYER_CHOICES_TXT][FOLD], true);
...
stock AddPlayerToTable(playerid, handle)
{
	...
	if( Character [ playerid ] [ character_handmoney ] < TableData[handle][E_TABLE_BUY_IN]) return SendPokerMessage(playerid, "You don't have enough money to access this table. Buy In: %s", cash_format(TableData[handle][E_TABLE_BUY_IN]));
	...
	//#warning For some reason, this is cancalled automatically. Find why this happens!!
	//SetPlayerCameraPos(playerid, TableData[handle][E_TABLE_POS_X], TableData[handle][E_TABLE_POS_Y], TableData[handle][E_TABLE_POS_Z]+T_Z_CAMERA_OFFSET);
	InterpolateCameraPos(playerid, TableData[handle][E_TABLE_POS_X], TableData[handle][E_TABLE_POS_Y], TableData[handle][E_TABLE_POS_Z]+T_Z_CAMERA_OFFSET, TableData[handle][E_TABLE_POS_X], TableData[handle][E_TABLE_POS_Y], TableData[handle][E_TABLE_POS_Z]+T_Z_CAMERA_OFFSET, 5000);

	//SetPlayerCameraLookAt(playerid, TableData[handle][E_TABLE_POS_X], TableData[handle][E_TABLE_POS_Y], TableData[handle][E_TABLE_POS_Z]);
	InterpolateCameraLookAt(playerid, TableData[handle][E_TABLE_POS_X], TableData[handle][E_TABLE_POS_Y], TableData[handle][E_TABLE_POS_Z], TableData[handle][E_TABLE_POS_X], TableData[handle][E_TABLE_POS_Y], TableData[handle][E_TABLE_POS_Z], 5000);

	ApplyAnimation(playerid, "INT_OFFICE", "OFF_Sit_Bored_Loop", 4.1, true, true, true, false, 0, SYNC_ALL);
	...
	TakeCharacterMoney(playerid, TableData[handle][E_TABLE_BUY_IN]);
...
public Poker_StartGame(handle, dealer)
{
	...
	foreach(new i: IT_PlayersTable<handle>)
	{
		...
		ApplyAnimation(i, "INT_OFFICE", "OFF_Sit_Bored_Loop", 4.1, true, true, true, false, 0, SYNC_ALL);
...
public Poker_KickPlayers(handle)
{
	for(new i = 0; i < MAX_PLAYERS; i++)
	{
		...
		if(!PlayerPokerData[i][E_PLAYER_TOTAL_CHIPS])
		{
			if( Character [ i ] [ character_handmoney ] < TableData[handle][E_TABLE_BUY_IN])
			{
				SendPokerMessage(i, "You don't have any chips left.");
				SendPokerMessage(i, "You may join the table again and pay the buy-in fee to play once again!");
				SendTableMessage(handle, ""COL_GREY"-- "COL_WHITE"Player %s has been kicked out of the table. [Reason: Ran out of chips]", ReturnPlayerName(i));
			...
			else
			{
				...
				await_arr(dialog_response) ShowPlayerAsyncDialog ( i, DIALOG_STYLE_MSGBOX, "Buy_In", ""COL_WHITE"You've ran out of chips. Do you want to pay the buy-in fee again to continue playing?", "Yes", "No" );
				...
				{
					TakeCharacterMoney(i, TableData[handle_ex][E_TABLE_BUY_IN]);
					PlayerPokerData[i][E_PLAYER_TOTAL_CHIPS] = TableData[handle_ex][E_TABLE_BUY_IN];
					SendTableMessage(handle_ex, ""COL_GREY"-- "COL_WHITE"Player %s has paid the buy-in fee of %s chips to keep playing.", ReturnPlayerName(i), cash_format(TableData[handle_ex][E_TABLE_BUY_IN]));
				}
				else
				{
					SendTableMessage(handle_ex, ""COL_GREY"-- "COL_WHITE"Player %s has been kicked out of the table. [Reason: Failure to pay the buy-in fee]", ReturnPlayerName(i));
					KickPlayerFromTable(i);
				}
			}
		}
...
static stock UpdateTable(handle)
{
	foreach(new playerid: IT_PlayersInGame<handle>)
	{
		...
		new str[128	];
		format(str, sizeof(str), "{34c5db}Chips: {db8d34}%s\n{db3a34}Last bet: {db8d34}%s", cash_format(PlayerPokerData[playerid][E_PLAYER_TOTAL_CHIPS]), cash_format(PlayerPokerData[playerid][E_PLAYER_CURRENT_BET]));
	...
	foreach(new pot_id: IT_Sidepots[handle])
	{
		...
		if(count == 1)
		{
			foreach(new p: It_SidepotMembers[_IT[handle][pot_id]])
			{
				...
				ApplyAnimation(p, "INT_OFFICE", "OFF_Sit_Crash", 4.1, false, true, true, true, 0, SYNC_ALL);
			}
...
{
	ApplyAnimation(playerid, "INT_OFFICE", "OFF_Sit_Crash", 4.1, false, true, true, true, 0, SYNC_ALL);
...
{
	ApplyAnimation(playerid, "INT_OFFICE", "OFF_Sit_Idle_Loop", 4.1, true, true, true, false, 0, SYNC_ALL);
	KillPlayerTurnTimer(playerid);
...
stock ShowPlayerRaiseDialog(playerid)
{
	...
	new dialog_response [ e_DIALOG_RESPONSE_INFO ] ;
	await_arr ( dialog_response ) ShowPlayerAsyncDialog ( playerid, DIALOG_STYLE_INPUT, "{FF8000}Input", sprintf("{FFFFFF}Please input the desired amount of chips: \n{FFFFFF}You may type {FF8000}%d {FFFFFF} if you want to go All In\n",  PlayerPokerData[playerid][E_PLAYER_TOTAL_CHIPS]), "Submit", "Cancel" );
	...
	if ( dialog_response [ E_DIALOG_RESPONSE_Response ] ) {
		...
		new raise = 0;
		if(sscanf(dialog_response[E_DIALOG_RESPONSE_InputText], "d", raise))
		{
...
hook OnPlayerDeathEx( playerid, killerid, WEAPON: reason, Float: damage, bodypart )
#else
...
CMD:ctable(playerid, params[])
{
	...
	{
		SendServerMessage(playerid, "/ctable [number of seats] [small blind] [buy in]",MSG_TYPE_INFO);
		SendServerMessage(playerid,"Creating a table will save in the database but deleting one will only delete it until next restart, please keep this in mind.",MSG_TYPE_WARN);
		return 1;
	}
	//Assumes the player is on a flat surface <Pos[2]-0.6>
	new Float:Pos[3];
	GetPlayerPos(playerid, Pos[0], Pos[1], Pos[2]);
	new table = CreatePokerTable(buy_in, small_blind, Pos[0], Pos[1], Pos[2]-0.6, seats, GetPlayerVirtualWorld(playerid), GetPlayerInterior(playerid),1);
...
{
	if(!IsPlayerModerator ( playerid )) {
		SendServerMessage(playerid, "You're not a moderator.", MSG_TYPE_ERROR);
	}
	if ( GetStaffGroup ( playerid ) < ADVANCED_MOD ) {
